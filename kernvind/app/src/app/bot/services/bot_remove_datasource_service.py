...
async def bot_remove_datasource(user_id:int, bot_id: int):
    try:
        botxrefs =  await BotUserXref.filter(user_id=user_id, bot_id=bot_id, can_change_datasource=True)
        if len(botxrefs) == 0:
            return Failure(error="User is Unauthorized to delete datasource from bot"), 401
        if len(botxrefs) > 1:
            return Failure(error="Something went wrong, please try again"), 500
        
        await Bot.filter(bot_id=bot_id).update(datasource_id=None)
